...
code of other members of the class, just as if it were a non-private member.
...
You can use the C<private> attribute to create a class
...
You cannot access private members from a subclass.
...
The C<-exclude> flag can describe a whole range of esoteric restrictions:
	...
	mem3 => '%',
	new => { post => '%mem3 = map { $_ => $mem1 } @m2' }
    ], -exclude => 'undef_ mem2_size mem3';
    $o = More_Examples->new mem1 => 1, mem2 => [2, 3, 4];
    @keys = $o->mem3_keys;	# Run-time error.
    $o->undef_mem1;		# Ditto.
    print $o->last_m2;		# This works as expected.
    print $o->m2_size;		# But this blows up.
...
you can specify it using the C<instance_var> option:
	...
	bar_mem => {
	    type => '$',
	    pre => 'print "foo_mem is ", $this->foo_mem;'
	}
    }, -parent => 'Foo',
...
whether warnings are in effect (see L<SANITY CHECKING>).
...
But go slow on that.
Remember, you can still make mistakes in code that invokes a class.
...
=item %s: "required" attribute ignored for private member "%s"

(W) A member cannot be both private and required as a constructor parameter.
...
=item %s: Evaluation failed (missing final semicolon in pre/post code?)

(F) Usually this error occurs if a class member has
a C<pre> or C<post> code fragment that does not end with a semicolon.
The other cause is an error in C<Class::Generate>.
...
=item %s::new: "%s" is not a member

(F) The named class uses the key/value parameter passing style.
Its constructor was invoked with a key/value pair with a key that
doesn't name a member of the class.
This message often occurs if you forget to use the key/value style.
...
Note this only affects code checking,
